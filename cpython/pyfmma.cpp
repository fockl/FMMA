...
inline void make_python(py::module &m, const std::string& type_str){
  ...
  py::class_<Class>(m, pyname.c_str())
    ...
    .def("set_fn1", static_cast<void (Class::*)(const std::function<TYPE(const std::array<TYPE, DIM>& target_source)>& fn)>(&Class::set_fn))
    .def("set_fn2", static_cast<void (Class::*)(const std::function<TYPE(const std::array<TYPE, DIM>& target, const std::array<TYPE, DIM>& source)>& fn)>(&Class::set_fn))
