...
//{{{ test_nrnmm_sum2_r2
template<typename TYPE, std::size_t DIM>
bool test_nrnmm_sum2_r2(int ssize, int tsize, TYPE tol){
  std::vector<std::array<TYPE, DIM>> source(ssize), target(tsize);
  std::vector<TYPE> source_weight(ssize);
  for(int i=0; i<ssize; ++i){
    for(std::size_t dim=0; dim<DIM; ++dim){
...
int main(void){
  ...
  if(!test_nrnmm_sum2_r2<double, 1>(10, 10, 1.0e-3)){
    exit(EXIT_FAILURE);
  }
  if(!test_nrnmm_sum2_r2<double, 1>(10, 20, 1.0e-3)){
    exit(EXIT_FAILURE);
  }
  ...
  if(!test_nrnmm_sum2_r2<double, 2>(10, 20, 1.0e-3)){
    exit(EXIT_FAILURE);
  }
